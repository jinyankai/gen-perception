...
| one-batch forward/backward | passed for all three tasks with an injected tiny U-Net/scheduler | real SD2 CPU and CUDA forward passed in S002/S003; real backward pending |
| 10-step loss and small-sample overfit | not run | pending |
| checkpoint/resume and decoded inference | shared code and unit tests implemented | real checkpoint resume and decoded predictions pending |
| visualization and provenance capture | reconstruction, prediction, and condition tools implemented | real-run artifacts pending |
...
| D5-D11 | VAE, conditioner, denoiser, scheduler | component load test and shape checks | reusable loader, Visual Latent Pathway and annealed noise implemented; real backward pending |
...
| D19-D26 | minimum ablations | condition, noise scale, step/latency tables | condition switches/diagnostics and noise configs implemented; runs pending |
